...
export type FormatType = "pretty" | "compact" | "plan";
...
export async function printResults(results: LintResult[], options: PrintOptions): Promise<void> {
  switch (options.format) {
    ...
    default:
      console.log();
      console.log(c.red(`${theme.icons.cross} Unknown format: "${options.format}"`));
      console.log();
      console.log(c.white(`${theme.icons.bullet} Available formats:`));
      console.log(c.cyan(`   ${theme.icons.check} pretty  - Detailed output with code snippets (default)`));
      console.log(c.cyan(`   ${theme.icons.check} compact - One line per issue`));
      console.log(c.cyan(`   ${theme.icons.check} plan    - Generate Markdown report for AI agents`));
      console.log();
      process.exit(1);
  }
}
...
function printPrettyBiome(results: LintResult[], options: PrintOptions): void {
  ...
  console.log(c.cyan(`${theme.icons.magnifying} ${c.bold("Analysis Results")}`));
  console.log();
  ...
  for (const [file, diagnostics] of byFile) {
    ...
    for (const diagnostic of diagnostics) {
      ...
      const severityIcon = diagnostic.severity === "error" ? theme.icons.error : diagnostic.severity === "warn" ? theme.icons.warning : theme.icons.info;
      const headerColor = diagnostic.severity === "error" ? c.red : diagnostic.severity === "warn" ? c.yellow : c.cyan;

      console.log(`${c.dim(theme.icons.corner)} ${headerColor(relativePath)}:${line}:${column}`);
      console.log(`  ${c.dim(theme.icons.bullet)} ${c.white(diagnostic.ruleId)}`);
      console.log();
      ...
      if (codeContext && codeContext.length > 0) {
        ...
        for (const ctx of codeContext) {
          ...
          console.log(`  ${lineColor(prefix)} ${c.dim(lineNum)} │ ${lineContent}`);
        }
      }
      ...
      if (diagnostic.suggestions?.length) {
        console.log(`  ${theme.icons.lightbulb} ${diagnostic.suggestions[0].desc}`);
        console.log();
      }

      if (diagnostic.fix) {
        console.log(`  ${theme.icons.wrench} ${c.green("Fix available (use --fix to apply)")}`);
        console.log();
      }
    }
  }
...
function getCodeContext(file: string, errorLine: number, contextLines: number = 2): Array<{ line: number; code: string }> {
  try {
...
function printPrettySuccess(options: PrintOptions): void {
  console.log();
  console.log(c.green.bold(`${theme.icons.sparkles} ${theme.icons.success} No issues found ${theme.icons.sparkles}`));
  console.log();
  console.log(`${theme.icons.file} ${c.white(options.filesAnalyzed)} files analyzed`);
  console.log(`${theme.icons.clock} ${c.dim(options.duration)}ms`);
  console.log();
  console.log(c.cyan(`${theme.icons.rocket} Your code is vibecode-free! ${theme.icons.rocket}`));
  console.log();
}
...
function printCompact(results: LintResult[], options: PrintOptions): void {
  ...
  if (allDiagnostics.length === 0) {
    console.log();
    console.log(c.green.bold(`${theme.icons.success} No issues found ${theme.icons.success}`));
    console.log();
    return;
  }
  ...
  for (const diagnostic of allDiagnostics) {
    ...
    console.log(`${severityPrefix} ${file}:${diagnostic.line}:${diagnostic.column}  ${c.cyan(diagnostic.ruleId)}  ${c.white(truncatedMessage)}`);
  }
  ...
  const summaryColor = totalErrors > 0 ? c.red : totalWarnings > 0 ? c.yellow : c.green;
  ...
  console.log(summaryColor(`  ${theme.icons.bug} ${totalErrors} ${errorLabel}  •  ${theme.icons.warningSign} ${totalWarnings} ${warningLabel}  •  ${theme.icons.folder} ${filesWithIssues} ${fileLabel}`));
  console.log(c.dim(`  ${theme.icons.clock} ${options.duration}ms  •  ${theme.icons.file} ${options.filesAnalyzed} files`));
  console.log();
}
...
async function printPlan(results: LintResult[], options: PrintOptions): Promise<void> {
  ...
  if (allDiagnostics.length === 0) {
    lines.push(`## ✅ Result`);
    lines.push(``);
    lines.push(`No issues found! The codebase is clean of AI-generated patterns.`);
    lines.push(``);
    lines.push(`---`);
    lines.push(``);
    ...
    if (options.path) {
      ...
      console.log();
      console.log(c.green(`${theme.icons.success} Report saved to ${outputPath}`));
      console.log();
    }
    return;
  }
  ...
  lines.push(``);
  lines.push(`You are a expert code reviewer. Fix the following issues found in the codebase.`);
  lines.push(``);
  ...
  for (const [file, diagnostics] of byFile) {
    ...
    for (let i = 0; i < diagnostics.length; i++) {
      ...
      const severity = diagnostic.severity === 'error' ? '🔴 ERROR' :
                       diagnostic.severity === 'warn' ? '🟡 WARNING' : 'ℹ️ INFO';
      ...
      if (diagnostic.fix) {
        lines.push(`**Fix Available:** YES (run \`vibrant --fix\` to auto-apply)`);
        lines.push(``);
      }

      lines.push(`**Reference Code:**`);
      lines.push(`\`\`\`typescript`);
      lines.push(`// Fix location: ${relativePath}:${diagnostic.line}:${diagnostic.column}`);
      lines.push(`// Rule: ${diagnostic.ruleId}`);
  ...
  lines.push(``);
  lines.push(`Vibrant detects AI-generated code patterns (vibecoding) including:`);
...
function printSummary(totalErrors: number, totalWarnings: number, filesWithIssues: number, options: PrintOptions): void {
  console.log(`${theme.icons.file} ${c.white(options.filesAnalyzed)} files scanned`);
  console.log(`${theme.icons.globe} ${c.white(filesWithIssues)} files with issues`);
  console.log(`${theme.icons.clock} ${c.dim(options.duration)}ms`);
  console.log();
  ...
  if (totalErrors > 0) {
    console.log(summaryColor(`  ${theme.icons.bug} ${totalErrors} ${errorLabel}  •  ${totalWarnings} ${warningLabel}`));
  } else if (totalWarnings > 0) {
    console.log(summaryColor(`  ${theme.icons.warningSign} ${totalErrors} ${errorLabel}  •  ${totalWarnings} ${warningLabel}`));
  } else {
    console.log(summaryColor(`  ${theme.icons.shield} ${totalErrors} ${errorLabel}  •  ${totalWarnings} ${warningLabel}`));
  }

  const totalFixable = (options.fixableErrors || 0) + (options.fixableWarnings || 0);
  if (totalFixable > 0) {
    console.log();
    console.log(c.cyan(`  ${theme.icons.wrench} ${totalFixable} issues can be fixed with ${c.white("--fix")}`));
  }
